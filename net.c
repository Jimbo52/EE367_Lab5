...
for (i=0; i<switchLinkArray->numlinks; i++) {
   if (pipe(switchLinkArray->link[i].toHost) < 0) {
      printf("Creating pipe failed\n");
      return;
   }
   if (pipe(switchLinkArray->link[i].fromHost) < 0) {
      printf("Creating pipe failed\n");
      return;
   }

   /* Set the pipes to nonblocking */
   pflag = fcntl(switchLinkArray->link[i].toHost[0],F_GETFL);
   fcntl(switchLinkArray->link[i].toHost[0],F_SETFL, pflag | O_NONBLOCK);

   pflag = fcntl(switchLinkArray->link[i].toHost[1],F_GETFL);
   fcntl(switchLinkArray->link[i].toHost[1],F_SETFL, pflag | O_NONBLOCK);

   pflag = fcntl(switchLinkArray->link[i].fromHost[0],F_GETFL);
   fcntl(switchLinkArray->link[i].fromHost[0],F_SETFL, pflag | O_NONBLOCK);

   pflag = fcntl(switchLinkArray->link[i].fromHost[1],F_GETFL);
   fcntl(switchLinkArray->link[i].fromHost[1],F_SETFL, pflag | O_NONBLOCK);
}
}
...
for (i = 0; i < switchLinkArray->numlinks; i++) {
    if (i != switchid) {
      close(switchLinkArray->link[i].toHost[0]);
      close(switchLinkArray->link[i].toHost[1]);
      close(switchLinkArray->link[i].fromHost[0]);
      close(switchLinkArray->link[i].fromHost[1]);
     }
}

/* Close manager's side of the connection from host to manager */
close(switchLinkArray->link[switchid].fromHost[PIPEREAD]);

/* Close manager's side of the connection from manager to host */
close(switchLinkArray->link[switchid].toHost[PIPEWRITE]);
}
...
for(j = 0; j < numlinks; j++) {
	linkArray->link[j].uniPipeInfo.physIdSrc = linkbeg[j];
	linkArray->link[j].uniPipeInfo.physIdDst = linkend[j];
}
}
...
for (i=startindex; i<linkArray->numlinks; i++) {
...
if (startindex == 0 && index == linkArray->numlinks) 
   printf("Error1:  Can't find outgoing link for switch\n");
return index; 
}
...
if (index == linkArray->numlinks) 
   printf("Error1:  Can't find outgoing link for host\n");
return index; 
}
...
if (startindex == 0 && index == linkArray->numlinks) 
   printf("Error2:  Can't find outgoing link for switch\n");
return index; 
}
...
if (index == linkArray->numlinks) 
   printf("Error2:  Can't find outgoing link for host\n");
return index; 
}
