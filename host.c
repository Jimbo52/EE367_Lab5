...
hstate->sendPacketBuff.payload[i] = '\0';
   ...
   #ifdef DEBUG
   printf("Debug: %s", hstate->sendPacketBuff.payload);
   #endif
...
#ifdef DEBUG
                printf("Display contents of rcvPacketBuff:\n%s\n",hstate->rcvPacketBuff.payload);
#endif
                //fwrite(hstate->rcvPacketBuff.payload,1,hstate->rcvPacketBuff.length,fp);
                //fprintf(fp,"%s",hstate->rcvPacketBuff.payload);
                fputs(hstate->rcvPacketBuff.payload,fp);
